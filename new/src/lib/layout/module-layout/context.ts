...
import type { ModuleOption } from '$lib/layout/module-layout/types';
...
const [internal__setLayoutPath, internal__getLayoutPath] = createContext<string>();
...
export function getLink(
	...
	const link = options.type === 'module' ? `${modulePath}/${href}` : `${modulePath}/api/${href}`;
	const result = options.allowSelfLink || $page.url.pathname !== link ? link : undefined;
